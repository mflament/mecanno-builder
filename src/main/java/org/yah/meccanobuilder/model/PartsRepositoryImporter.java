package org.yah.meccanobuilder.model;
...
public class PartsRepositoryImporter {

    public static void main(String[] args) throws IOException {
        final PartsRepositoryImporter importer = new PartsRepositoryImporter();
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(PartsRepositoryImporter.class);
    ...
    public PartsRepositoryImporter() {
        repository = new PartsRepository();
    }

    private PartsRepository importRepository() throws IOException {
        importColors();
        importMaterials();
    ...
    private MeccanoSet[] parseSets(String[][] values) {
        ...
        for (int col = 7; col < values[0].length; col++) {
            final String s = trimToNull(values[0][col]);
            if (s != null)
                serie = s;
            if (serie == null)
            ...
            final String name = values[2][col].trim() + " " + values[3][col].trim();
    ...
    private Part[] parseParts(String[][] values) {
        ...
        for (int row = 6; row < values.length; row++) {
            ...
            Part part = repository.findPartsByName(partName).orElse(null);
            if (part == null) {
                final String partNumber = parsePartNumber(columns);
                part = Part.builder()
                        .withNumber(partNumber)
                        .withNewNumber(trimToNull(columns[2]))
                        .withName(partName)
                        .build();
